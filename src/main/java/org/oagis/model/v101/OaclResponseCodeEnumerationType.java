//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.8-b130911.1802 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2015.04.09 at 05:04:55 PM CST 
//


package org.oagis.model.v101;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for oacl_ResponseCodeEnumerationType.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="oacl_ResponseCodeEnumerationType">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}token">
 *     &lt;enumeration value="Always"/>
 *     &lt;enumeration value="OnError"/>
 *     &lt;enumeration value="OnModification"/>
 *     &lt;enumeration value="OnRejection"/>
 *     &lt;enumeration value="Never"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "oacl_ResponseCodeEnumerationType")
@XmlEnum
public enum OaclResponseCodeEnumerationType {

    @XmlEnumValue("Always")
    ALWAYS("Always"),
    @XmlEnumValue("OnError")
    ON_ERROR("OnError"),
    @XmlEnumValue("OnModification")
    ON_MODIFICATION("OnModification"),
    @XmlEnumValue("OnRejection")
    ON_REJECTION("OnRejection"),
    @XmlEnumValue("Never")
    NEVER("Never");
    private final String value;

    OaclResponseCodeEnumerationType(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static OaclResponseCodeEnumerationType fromValue(String v) {
        for (OaclResponseCodeEnumerationType c: OaclResponseCodeEnumerationType.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
